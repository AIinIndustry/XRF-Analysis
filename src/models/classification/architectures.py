...
class MLPCountClassifier(torch_nn.Module):
    def __init__(self, input_dim=600, num_classes=5):
        super().__init__()
        self.network = torch_nn.Sequential(
            torch_nn.Linear(input_dim, 512),
            torch_nn.ReLU(),
...
class CNNCountClassifier(torch_nn.Module):
    def __init__(self, input_dim=600, num_classes=5):
        super().__init__()
        self.encoder = torch_nn.Sequential(
            torch_nn.Conv1d(1, 16, kernel_size=3, padding=1),
        ...
        self.classifier = torch_nn.Sequential(
            torch_nn.Flatten(),
            torch_nn.Linear(32 * (input_dim // 4), 128),
            torch_nn.ReLU(),
            torch_nn.Dropout(0.2), 
            torch_nn.Linear(128, num_classes),
            torch_nn.Sigmoid()
...
class TransformerCountClassifier(torch_nn.Module):
    def __init__(self, input_dim=600, num_classes=5, d_model=64, nhead=4, num_layers=2):
        super().__init__()
        self.input_projection = torch_nn.Linear(1, d_model)
