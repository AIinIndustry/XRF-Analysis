...
# MAE data (Original Scale * 1000) — Standard MSE + L1 loss
mae_data = {
    "Standard":[1.25, 1.22, 1.34, 1.66, 1.44, 1.72, 1.20, 1.32],
    "Min-Max": [1.56, 2.40, 1.29, 1.71, 1.35, 1.70, 1.38, 1.56],
    "Log+MinMax": [1.43, 1.30, 1.27, 1.64, 1.40, 1.68, 1.39, 1.51]
}

# MSE data (Original Scale * 10^5) — Standard MSE + L1 loss
mse_data = {
    "Standard":[3.58, 3.93, 4.13, 4.19, 4.36, 4.37, 4.88, 4.94],
    "Min-Max": [4.71, 11.70, 4.24, 5.12, 4.55, 5.21, 4.46, 4.86],
    "Log+MinMax": [4.38, 4.45, 4.40, 4.77, 4.54, 4.92, 4.63, 4.74]
}
...
def create_grouped_bar(data, architectures, title, ylabel, save_path, reverse=True):
    ...
    plt.bar(x, [data["Min-Max"][i] for i in sort_idx], width, label='Min-Max', color='#e74c3c', edgecolor='darkred', alpha=0.8)
    plt.bar(x + width, [data["Log+MinMax"][i] for i in sort_idx], width, label='Log+MinMax', color='#2ecc71', edgecolor='darkgreen', alpha=0.8)
...
create_grouped_bar(mae_data, architectures,
                  'Denoising MAE by Preprocessing Strategy (Standard MSE + L1 loss)',
                  'MAE (Original Scale, ×10⁻³)',
                  'Slide/media/denoising_mae_comparison.png')

create_grouped_bar(mse_data, architectures,
                  'Denoising MSE by Preprocessing Strategy (Standard MSE + L1 loss)',
                  'MSE (Original Scale, ×10⁻⁵)',
                  'Slide/media/denoising_mse_comparison.png')
